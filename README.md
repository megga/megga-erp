...
Le **réapprovisionnement** ferme le cycle : le magasin se remplit tout
...
commandes sont entièrement dans Community. **Aucun cron maison** — le
cœur en a déjà un (`stock.ir_cron_scheduler_action`), et deux
ordonnanceurs qui se croisent font des bons de commande en double. Le
bon de commande reste un **brouillon** : rien ne part chez le
fournisseur sans un geste du cabinet.
...
Le raccourci du cabinet est volontairement une **action fenêtre
filtrée**, et non l'écran de réapprovisionnement complet du cœur : ce
dernier sème des règles manuelles pour tous les produits en manque de
la société, restaurant et garage compris. Le menu d'un cabinet n'a pas
à décider du réassort des autres verticales.
...
Le **registre du matériel** (`megga_dental_materiel`) est le pendant du
magasin : le magasin **compte ce qui se consomme**, le registre
**entretient ce qui dure**. Autoclave, compresseur, générateur de
rayons X, unit : ce qui se révise, se calibre et se prouve. Module
**séparé, jamais auto-installé** — un cabinet peut vouloir l'un sans
l'autre.

Comme le magasin, il **configure le cœur** bien plus qu'il ne modélise.
Le module `maintenance` de Community tient déjà tout : numéro de série,
fournisseur et sa référence, date de mise en service, fin de garantie,
coût, technicien responsable, équipe, historique des pannes, MTBF et
MTTR, et l'entretien préventif périodique. Il ne manquait au cabinet
qu'une chose, et le module n'ajoute qu'elle : **le fauteuil**. Un
cabinet ne cherche pas « l'autoclave 3 », il cherche *ce qu'il y a
autour du fauteuil 2* — pour savoir ce qui s'arrête quand un appareil
part en réparation. D'où l'écran groupé par fauteuil, l'onglet
« Matériel » de la fiche fauteuil, et un lien `ondelete='restrict'` :
un fauteuil qui porte du matériel ne se supprime plus, il **s'archive**
(doctrine du dépôt pour tout référentiel porteur d'histoire).

L'entretien périodique est celui du cœur, sans une ligne de plus : une
demande **récurrente** (validation trimestrielle de l'autoclave,
révision annuelle du compresseur) engendre la suivante **quand on la
clôt** — le cœur la régénère dans le `write` sur l'étape terminale.
**Aucun cron maison, aucune périodicité maison.** Une panne, elle, ne
se reproduit pas : un correctif clos ne programme rien.

Les droits sont ceux du cœur, et ils ne sont pas ceux qu'on croit —
c'est la leçon de ce chantier. L'ACL donne bien la **lecture** des
équipements à tout employé, mais une règle d'enregistrement
(`maintenance.equipment_rule_user`) la borne aux équipements dont il
est **suiveur** : c'est le modèle « mon ordinateur portable », pas « le
registre de la maison ». Un praticien ordinaire aurait ouvert un menu
« Appareils » **vide**. Le registre complet va donc au **gestionnaire
d'équipements** ; « Entretiens » reste ouvert à tout employé —
signaler une panne au fauteuil est le geste du praticien, et le cœur
l'ouvre à tous. La même règle a valu une seconde correction, trouvée
par la revue : le compteur d'appareils de la fiche fauteuil se calcule
en `sudo` (un nombre est de la logistique) alors que la liste qu'il
ouvre passe, elle, par la règle du cœur — le praticien lisait
« 1 appareil » sur un onglet vide. Bouton et onglet portent désormais
la même garde que le menu, et un test tient l'accord entre le compteur
et l'écran qu'il ouvre.

Le décor livré est du **classement, pas du matériel** : une équipe
...
sont des lots datés du **magasin**, dans une catégorie **enfant** des
consommables du cabinet — enfant pour être visible partout où le
cabinet regarde (les écrans du magasin, et surtout le sélecteur de kit
d'une position tarifaire), et portant quand même sa propre stratégie
FEFO, que le cœur ne fait pas hériter. La péremption de stérilité est la
...
est **figée**, comme une ordonnance émise — ses lignes de charge
comprises, sans quoi le registre pourrait dire autre chose que ce qui
est sorti de l'autoclave. Deux champs restent ouverts après coup :
l'**indicateur biologique**, dont le résultat arrive plus tard — et le
passer à « non conforme » **bloque** la charge à lui seul, sans qu'on
ait à cliquer quoi que ce soit — et les **observations**, où l'on note
ce qu'on a fait de la charge. L'état, lui, ne se force pas par un
`write` : il se change par les boutons, qui font chacun ce qu'ils
annoncent. Une charge validée ne revient pas au
brouillon non plus : ses sets sont partis en rayon, peut-être au
fauteuil. On repasse la charge, on ne réécrit pas l'histoire.

**Qui tient le registre.** Le menu est gardé par le **gestionnaire
d'équipements** du cœur, pas par les groupes dentaires — et c'est la
leçon du registre du matériel, que ce chantier avait rouverte avant
qu'une revue ne la referme. Le cycle exige un autoclave ; l'ACL du cœur
en ouvre bien la lecture à tout employé, mais une règle
d'enregistrement la borne aux équipements dont on est **suiveur**. Une
assistante n'ayant que le groupe Réception aurait ouvert le formulaire,
cherché son autoclave dans une liste **vide**, et n'aurait pas pu
enregistrer une seule charge. Le registre revient donc au responsable
technique — celui qui tient déjà le matériel, et à qui la validation
périodique de l'autoclave incombe de toute façon. Un cabinet qui veut
que ses assistantes saisissent les charges leur donne ce droit : un
geste d'administration délibéré, comme les droits stock du magasin. Le
module n'invente aucun groupe et ne câble aucun `implied_ids`. La
réception, elle, **lit** les cycles — la preuve d'une séance en
dépend — sans pouvoir en enregistrer.

L'entrée en rayon reste un **effet système du flux**, en `sudo`,
exactement comme la clôture de séance décompte les consommables : celui
qui valide la charge n'a **aucun droit sur le magasin** — il ne peut
pas créer un lot à la main, et la validation en crée pour lui sans lui
en donner le droit. Conséquence assumée : la page « Sets produits » de
la fiche est gardée par le groupe stock du cœur, sinon elle refuserait
de s'ouvrir à celui qui vient de valider ; la **date de stérilité**,
elle, s'affiche sur la ligne de charge — lisible sans ouvrir
`stock.lot` (ce qui aurait ouvert les lots du restaurant avec) et lue
sur le **lot** dès qu'il existe, pour qu'il n'y ait jamais deux dates
qui divergent.

**Le stock ne bloque jamais la clinique — y compris ici.** Une garde
qui refuse une sortie depuis `_action_done` tire sur le *soin* si rien
ne retire la ligne fautive avant : la séance ne se clôture plus. Le
chantier 2 avait écrit cette ceinture pour le périmé ; la garde de
stérilisation est arrivée sans la sienne, et la revue l'a rattrapée
avant le premier cabinet. La ceinture est désormais un **point
d'extension** du magasin (`_megga_unservable_lines`) que les deux
gardes alimentent : le set non conforme est écarté de la réservation,
la ligne repart sans lot, et l'activité au magasin nomme le cycle.
Toute garde ajoutée plus tard devra faire de même.

**Une revue de code a trouvé onze défauts de plus**, tous corrigés — et
deux méritent d'être connus parce qu'ils ne se voient pas. Le cœur
interpose un assistant de confirmation dès qu'une ligne porte un lot
dont la **date de retrait** est atteinte, pas seulement la péremption :
un set qui sort de l'autoclave pouvait donc être « périmé » à
l'entrée, l'assistant remplaçait la validation, et la charge devenait
**définitivement** impossible à valider (vérifié par mutation). Et le
re-parentage de la catégorie des sets vivait dans un bloc
`noupdate="1"` : il ne se serait **jamais appliqué à une base déjà
installée** — les tests passaient parce que la CI installe à neuf. Un
script de migration le rattrape, vérifié sur une vraie montée de
version.

**Deux limites connues**, documentées parce qu'elles ne s'inventent
pas. Le rappel remonte aux séances par le **transfert de
consommation** : un set sorti à la main, hors clôture de séance, n'y
figure pas — le message du rappel le dit. Et la **péremption de
stérilité n'existe que si le produit porte un délai** : sans
`expiration_time` réglé sur le set, le cœur ne date rien, ni FEFO ni
garde ne mordent. L'écran le signale désormais en orange sur la ligne
de charge, plutôt que de le laisser au seul README.
